...
- **Overall: ≥80%** — configured in `pyproject.toml`.
  reads `coverage.json` (produced by the `--cov-report=json` addopt).

Both gates run in CI (`.github/workflows/tests.yml`) on every push to `main` and
every pull request. New code should ship with tests that keep both gates green.
...
3. Ensure `uv run pytest` and the per-module coverage check both pass.
...
- Run the full test suite **and** the per-module coverage check before declaring a
  task done. Do not lower the coverage thresholds to make a change pass.
...
scripts/
    check_module_coverage.py  # per-module coverage gate (>=70%)
```
...
| GET | `/job/<id>/progress` | Job progress (`total`/`completed`/`status`) |
